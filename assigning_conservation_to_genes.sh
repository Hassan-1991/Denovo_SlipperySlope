...
grep "^" /stor/scratch/Ochman/hassan/RefSeq_Complete_Genomes_04062024/04062024_RS_GB_complete_bacterial_genomes/10082024_noncoli_Escherichia_database/gtfs/*gtf | cut -f1 | rev | cut -f1 -d "/" | rev | sed "s/.gtf:/\t/g" | awk -F '\t' '{OFS=FS}{print $2,$1}' | awk -F'\t' '{if ($2 ~ /GCA_/) sub(/_[^_]+$/, "", $2); print $0}' OFS='\t' | sort -u > Escherichia_contig_accession.tsv
